...
run(int argc, char **argv) {
	// save argv to string
	string input_video = argv[1], calibration_file = argv[2],
		   LK_file = argv[3], output_path = argv[4];
	...
	if (!capture.isOpened()) {
		...
		summary_temp << " - File name: " << input_video << endl;
	}
	...
	float pixel_to_real, partial_min_angle, partial_max_angle, final_min_angle,
		final_max_angle, final_min_distance, max_features, region_step,
		resolution;

	int radius, maxLevel, maxCount, flags;
	double minEigThreshold, epsilon;

	readLKParameters(LK_file,
					 radius,
					 maxLevel,
					 maxCount,
					 epsilon,
	...
	readCalibration(calibration_file,
					pixel_to_real,
					partial_min_angle,
					partial_max_angle,
					final_min_angle,
	...
	summary_temp << " * FILTERING SETTINGS" << string(2, '\n')
				 << " - Partial filtering (min/max angle): "
				 << partial_min_angle << "° to " << partial_max_angle << "°"
				 << endl
				 << " - Final filtering (min/max angle): " << final_min_angle
				 << "° to " << final_max_angle << "°" << endl
				 << " - Final filtering (min covered distance): "
				 << final_min_distance << " pixels" << endl
				 << string(2, '\n');
	...
	summary_temp << " * STARTING COMPUTATION" << string(2, '\n')
				 ...
				 << " - Resolution selected: " << resolution_s << " (" << width
				 << "x" << height << ")" << endl
				 << detected_mask << endl
				 << string(2, '\n');
	...
	while (progress < 1.0) {
		...
		if (resolution != 1.0) {
			resize(current_frame,
				   current_frame,
				   Size(current_frame.cols * resolution,
						current_frame.rows * resolution));
		...
		if (!previous_frame.empty()) {
			...
			calcOpticalFlowPyrLK(previous_frame,
								 current_frame,
								 pt1,
								 pt2,
								 status,
			...
			for (i = k = 0; i < keypoints_current.size(); i++) {
				if (!status[i]
					|| !partialFiltering(keypoints_current[i],
										 keypoints_predicted[i],
										 partial_min_angle,
										 partial_max_angle,
										 maxLevel * (2 * radius + 1)
											 / resolution)) {
					if (finalFiltering(keypoints_start[i],
									   keypoints_current[i],
									   final_min_distance,
									   final_min_angle,
									   final_max_angle)) {
						float velocity_i
							= getVelocity(keypoints_start[i],
										  keypoints_current[i],
										  pixel_to_real / resolution,
										  frame_idx - time[i],
										  fps);
						float angle_i = getAngle(keypoints_start[i],
												 keypoints_current[i]);

						// sub-regions computation
						int module_start = (int)keypoints_start[i].pt.x / step,
							module_current
							= (int)keypoints_current[i].pt.x / step;

						if (module_start == module_current) {
							subregions_velocity[module_start].push_back(
								velocity_i);
						...
						distance[frame_idx].push_back(
							velocity_i * (frame_idx - time[i]) / fps);
					}
	...
	if (count != 0) {
		summary_temp << " - Execution Time: " << execution_time << " s" << endl
					 << " - Computational Frame Rate: "
					 << total_frames / execution_time << " f/s" << endl
					 << " - Max Velocity: " << max << " m/s," << endl
					 << " - Min Velocity: " << min << " m/s," << endl
					 << " - Avg Velocity: " << avg << " m/s," << endl
					 << " - Std Deviation: " << std_dev << " m/s," << endl
					 << " - Tot Filtered Trajectories: "
					 << floatToString(count, 1) << string(2, '\n');
	} else {
		summary_temp << " - Execution Time: " << execution_time << " s" << endl
					 << " - Computational Frame Rate: "
					 << total_frames / execution_time << " f/s" << endl
	...
	vector<float> averages
		= getSubRegionAvg(subregions_velocity, subregions_trajectories);
	vector<float> std = getSubRegionStd(subregions_velocity, averages);
	writeSubRegionStats(averages,
						subregions_trajectories,
						std,
						width,
						step,
						output_path);
...
main(int argc, char **argv) {
	if (argc != 5 && argc != 6) {
		cout << "Usage: " << argv[0] << " [input video path]"
			 << " [input calibration file]"
			 << " [input LK parameters file]"
			 << " [output path]"
			 << " [mask path (optional)]" << endl;
	} else {
		run(argc, argv);
	}
